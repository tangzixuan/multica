...
For other runtimes (Hermes / Kimi / Kiro / Copilot / Cursor / Gemini / Pi / OpenCode / OpenClaw) the daemon leaves the user's `$HOME` untouched and lets the CLI discover user-level skills natively from its own conventional path (e.g. `~/.copilot/skills/`, `~/.cursor/skills/`). The toggle is a no-op for these runtimes today; a future change can extend per-runtime isolation as upstream CLIs ship the necessary knobs.
