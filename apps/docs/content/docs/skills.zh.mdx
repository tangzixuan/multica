...
对其它运行时（Hermes / Kimi / Kiro / Copilot / Cursor / Gemini / Pi / OpenCode / OpenClaw），守护进程不改写用户的 `$HOME`，由 CLI 自己沿用约定路径原生发现用户级 skill（例如 `~/.copilot/skills/`、`~/.cursor/skills/` 等）。这些运行时今天对开关属于 no-op；待上游 CLI 提供必要的隔离开关后，可以再补齐。
